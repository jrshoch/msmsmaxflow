...
public class GeneratePlanarGraph {
	...
	List<Vertex> vertices = convertVerticesToList(graph);
	...
	List<Vertex> output = new ArrayList<Vertex> (vertices.size());
    ...
    protected static Map<Vertex,Vertex> randomWalkFromSourceToSink(Graph graph, 
	    Vertex s, Vertex t){
    ...
    protected static void addRandomCapacityOnRandomWalk(Graph graph, Vertex s, Vertex t,
	    long maxCapacity) throws NoPathExistsException{
	Map<Vertex,Vertex> predecessors = randomWalkFromSourceToSink(graph, s, t);
    ...
    protected static void addCapacitiesFromRandomWalks(Graph graph, Vertex s, Vertex t,
	    long maxCapacity, long numPaths) throws NoPathExistsException{
    ...
    protected static List<Vertex> getNextFrontier(Graph graph, long maxCapacity, 
	    List<Vertex> currentFrontier, Set<Vertex> homeHalf, 
	    Set<Vertex> awayHalf, Set<Edge> edgesInCut){
	Edge edge;
	long newCapacity;
	Random rand = new Random();
	...
	for (Vertex tailVertex : currentFrontier){
		...
		if (!homeHalf.contains(headVertex)){
			...
			// The headVertex is in the other half, which means we've reached
			// one part of the cut.
			edgesInCut.add(edge);
		    } else {
			// Continue moving forward and add some capacity to the edge
			newCapacity = edge.getCapacity() + (long)
				(rand.nextDouble()*maxCapacity);
